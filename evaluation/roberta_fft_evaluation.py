...
def sliding_tokenize(text, tokenizer, max_length=128, stride=64):
    enc = tokenizer(
        text, 
        truncation=True, 
    ...
    return list(zip(enc["input_ids"], enc["attention_mask"]))

def tokenize_function(batch, tokenizer):
    inputs, masks, labels = [], [], []
    for text, label in zip(batch["text"], batch["label"]):
        for ids, mask in sliding_tokenize(text, tokenizer):
            inputs.append(ids); masks.append(mask); labels.append(label)
    return {"input_ids": inputs, "attention_mask": masks, "label": labels}
...
def main():
    ...
    tokenizer = BertTokenizer.from_pretrained(BASE_MODEL_NAME, use_fast=True)
    ...
    OUTPUT_DIR = f"./results/{dataset_stem}_roberta_fft_evalation_results"
    ...
    with tqdm(total=len(batch_indices), desc="Inference batches") as pbar:
        for start_idx in batch_indices:
            ...
            for text, gt, pred, conf in zip(texts, labels, preds, confs):
                totals["total"] += 1; totals["correct"] += pred == gt
                if gt == 0:
                    totals["label0_total"] += 1; totals["label0_correct"] += pred == gt
                else:
                    totals["label1_total"] += 1; totals["label1_correct"] += pred == gt
                y_true.append(gt); y_pred.append(pred)
                rec = dict(input=text, ground_truth=gt, predicted=pred, confidence=conf)
                details.append(rec)
