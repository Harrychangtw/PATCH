...
def main():
    ...
    print("Tokenized dataset cached to cache/test.arrow")
    dataset_stem = os.path.splitext(os.path.basename(DATASET_PATH))[0]
    OUTPUT_DIR = f"./results/{dataset_stem}_roberta_lora_evaluation_results"
    ...
    with tqdm(total=len(batch_indices), desc="Inference batches") as pbar:
        for start_idx in batch_indices:
            ...
            for txt, gt, pred, conf in zip(texts, labels, preds, confs):
                totals["total"] += 1
                ...
                y_true.append(gt)
                y_pred.append(pred)
                rec = dict(input=txt, ground_truth=gt, predicted=pred, confidence=conf)
                details.append(rec)
                if pred != gt:
                    errors.append(rec)
            pbar.update(1)
